...
public class WeatherService {
	...
	public JSONObject getWeather() throws Exception {
		...
		weather.setWeather_serviceKey(
				"Q3Jb4VLoiLZHahN%2BoYfI%2BerBplbbZ4%2BNN2XrxdjqZ3hNy%2BuAujVtG3s%2F%2BPWq9BiLB5Anfysmb4Kltjhe0bFUCQ%3D%3D");// serviceKey
		...
		weather2.setWeather_serviceKey(
				"Q3Jb4VLoiLZHahN%2BoYfI%2BerBplbbZ4%2BNN2XrxdjqZ3hNy%2BuAujVtG3s%2F%2BPWq9BiLB5Anfysmb4Kltjhe0bFUCQ%3D%3D");// serviceKey
		...
		weather3.setWeather_serviceKey(
				"Q3Jb4VLoiLZHahN%2BoYfI%2BerBplbbZ4%2BNN2XrxdjqZ3hNy%2BuAujVtG3s%2F%2BPWq9BiLB5Anfysmb4Kltjhe0bFUCQ%3D%3D");// serviceKey
	...
	public JSONArray getjsondata(WeatherDTO weather) throws Exception {
		...
		urlBuilder.append("&" + URLEncoder.encode("pageNo", "UTF-8") + "="
				+ URLEncoder.encode(weather.getWeahter_pageNo() + "", "UTF-8")); /* 페이지번호 */
		urlBuilder.append("&" + URLEncoder.encode("numOfRows", "UTF-8") + "="
				+ URLEncoder.encode(weather.getWeather_numOfRows() + "", "UTF-8")); /* 한 페이지 결과 수 */
		urlBuilder.append("&" + URLEncoder.encode("dataType", "UTF-8") + "="
				+ URLEncoder.encode(weather.getWeather_dataType() + "", "UTF-8")); /* 요청자료형식(XML/JSON)Default: XML */

		if (weather.getWeather_url().equals("VilageFcstInfoService_2.0/getVilageFcst")) {
			urlBuilder.append("&" + URLEncoder.encode("base_date", "UTF-8") + "="
					+ URLEncoder.encode(weather.getWeather_base_date() + "", "UTF-8")); /* ‘21년 6월 28일발표 */
			urlBuilder.append("&" + URLEncoder.encode("base_time", "UTF-8") + "="
					+ URLEncoder.encode(weather.getWeather_base_time() + "", "UTF-8")); /* 05시 발표 */
			urlBuilder.append("&" + URLEncoder.encode("nx", "UTF-8") + "="
					+ URLEncoder.encode(weather.getWeather_nx() + "", "UTF-8")); /* 예보지점의 X 좌표값 */
			urlBuilder.append("&" + URLEncoder.encode("ny", "UTF-8") + "="
					+ URLEncoder.encode(weather.getWeather_ny() + "", "UTF-8")); /* 예보지점의 Y 좌표값 */

		} else {
			urlBuilder.append("&" + URLEncoder.encode("regId", "UTF-8") + "=" + URLEncoder.encode(
					weather.getWeather_redId() + "", "UTF-8")); /* 108 전국, 109 서울, 인천, 경기도 등 (활용가이드 하단 참고자료 참조) */
			urlBuilder.append(
					"&" + URLEncoder.encode("tmFc", "UTF-8") + "=" + URLEncoder.encode(weather.getWeather_tmFc() + "",
